...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = 'erivan.raposo@gmail.com') THEN
    ...
    INSERT INTO auth.users (
      ...
      new_user_id, '00000000-0000-0000-0000-000000000000', 'erivan.raposo@gmail.com', crypt('Skip@Pass123', gen_salt('bf')), NOW(),
      NOW(), NOW(), '{"provider": "email", "providers": ["email"]}', '{"name": "Erivan Raposo"}',
      false, 'authenticated', 'authenticated',
      '', '', '', '', '', NULL, '', '', ''
    );

    INSERT INTO public.workspaces (id, name, budget_mensal_usd)
    VALUES (new_user_id, 'Workspace Principal', 100.0)
    ON CONFLICT (id) DO NOTHING;

    INSERT INTO public.profiles (id, full_name, role, workspace_id)
    VALUES (new_user_id, 'Erivan Raposo', 'admin', new_user_id)
    ON CONFLICT (id) DO NOTHING;
  END IF;
END $$;
...
DROP VIEW IF EXISTS public.vw_recent_invocations CASCADE;

CREATE OR REPLACE VIEW public.vw_recent_invocations AS
SELECT 
  i.id,
  i.created_at,
  i.input_tokens,
...
CREATE POLICY "authenticated_select_profiles" ON public.profiles
  FOR SELECT TO authenticated USING (
    id = auth.uid() 
    OR workspace_id IN (SELECT workspace_id FROM public.profiles WHERE id = auth.uid())
  );
