...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          await supabase.from('invocacoes').upsert(initialInvocationPayload, { onConflict: 'id', ignoreDuplicates: true });
          ...
          if (action === 'apply') {
              ...
              if (anthropicKey) {
                  ...
                  if (agent.thinking_mode === 'enabled') {
                      payloadParams.thinking = { type: 'enabled', budget_tokens: Math.max(1024, Math.floor(maxTokens * 0.8)) };
                  }
                  ...
                  await supabase.from('invocacoes').update({ input_tokens: inputTokens, output_tokens: outputTokens }).eq('id', activeInvocationId);
              ...
              for (const agent of agentsToProcess) {
                  ...
                  if (anthropicKey) {
                      ...
                      if (!aiData.content || !Array.isArray(aiData.content) || aiData.content.length === 0) {
                        throw new Error(JSON.stringify({ error: "Erro: A resposta da IA estava vazia ou malformada.", code: "EMPTY_RESPONSE" }));
                      }
                      ...
                      if (textBlocks.length === 0) {
                        ...
                        throw new Error(JSON.stringify({ error: "Erro: A resposta da IA não continha blocos de texto (apenas thinking).", code: "EMPTY_RESPONSE" }));
                      }
                      ...
                      if (!aiText || aiText.length === 0) {
                        ...
                        throw new Error(JSON.stringify({ error: "Erro: A resposta da IA estava vazia após extração.", code: "EMPTY_RESPONSE" }));
                      }                      
                  ...
                  await supabase.from('invocacoes').update({ input_tokens: inputTokens, output_tokens: outputTokens }).eq('id', activeInvocationId);
                  await supabase.from('custos').upsert({ invocation_id: activeInvocationId, estimated_cost: estimatedCost, currency: 'USD', cached_tokens: cachedTokens }, { onConflict: 'invocation_id', ignoreDuplicates: true });
              }
          ...
          if (activeInvocationId && authHeader) {
              ...
              await safeClient.from('invocacoes').update({ diagnostic_log: err.message }).eq('id', activeInvocationId);
          }
    ...
    if (activeInvocationId && authHeader) {
        const safeClient = createClient(supabaseUrl, supabaseKey, { global: { headers: { Authorization: authHeader } } });
        await safeClient.from('invocacoes').update({ diagnostic_log: error.message }).eq('id', activeInvocationId);
    }
